...
const AdminDashboard = () => {
  ...
  const startPolling = () => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/admin/etl-status');
  ...
  const handleRunETL = async () => {
    try {
      await axios.post('http://localhost:8000/api/admin/run-etl');
  ...
  const handleReloadDB = async () => {
    try {
      const res = await axios.post('http://localhost:8000/api/admin/reload-db');
